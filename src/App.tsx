    ...
    <div className={`app-shell theme-${theme} accent-${accent}`}>
      <aside className={`sidebar ${sidebarOpen ? '' : 'sidebar-collapsed'}`}>
        ...
        <nav className="main-nav">
          {navItems.map(({ id, label, icon: Icon }) => <button key={id} className={`nav-item ${page === id ? 'active' : ''}`} onClick={() => navigate(id)}><Icon size={18} /><span>{sidebarOpen && label}</span>{id === 'chat' && sidebarOpen && chatUnread > 0 && <small>{chatUnread > 99 ? '99+' : chatUnread}</small>}</button>)}
        </nav>
